...
function KodePerihalPage() {
    ...
    const fetchData = async () => {
        try {
            const data = await apiGet('/kode-perihal');
            setItems(data.kode_perihals || []);
        } catch (error) {
            Swal.fire({
                icon: 'error',
                title: 'Gagal',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Swal.fire({
                ...
                title: 'Berhasil',
                text: editingId
                    ? 'Kode perihal berhasil diperbarui.'
                    : 'Kode perihal berhasil ditambahkan.',
                timer: 1600,
    ...
    const handleDelete = async (item) => {
        ...
        try {
            await apiSend(`/kode-perihal/${item.id}`, 'DELETE');
            await fetchData();

            Swal.fire({
                icon: 'success',
                title: 'Terhapus',
                text: 'Kode perihal berhasil dihapus.',
                timer: 1600,
                showConfirmButton: false,
            });
        } catch (error) {
    ...
    return (
        <div className="space-y-5">
            <div className="bg-white rounded-2xl shadow border border-slate-100 p-6 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">
                        Kelola kode perihal untuk format nomor surat.
                    </p>
                </div>
                    ...
                    <table className="w-full">
                        <thead className="bg-slate-50">
                            <tr>
                                ...
                                <th className="text-left px-5 py-4 text-sm font-bold text-slate-700">Nama Perihal</th>
                                <th className="text-left px-5 py-4 text-sm font-bold text-slate-700">Deskripsi</th>
                            ...
                            {items.length > 0 ? (
                                items.map((item, index) => (
                                    <tr key={item.id} className="border-t border-slate-100">
                                        ...
                                        <td className="px-5 py-4 font-bold text-red-700">{item.kode}</td>
                                        <td className="px-5 py-4">{item.nama_perihal}</td>
                                        <td className="px-5 py-4 text-slate-500">{item.deskripsi || '-'}</td>
            ...
            {modalOpen && (
                <Modal
                    title={editingId ? 'Edit Kode Perihal' : 'Tambah Kode Perihal'}
                    subtitle="Isi data kode perihal surat."
                    onClose={closeModal}
                >
...
export default KodePerihalPage;
